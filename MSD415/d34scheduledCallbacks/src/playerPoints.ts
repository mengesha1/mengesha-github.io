...
function calculateTotalPoints(player: Player): JerseyPoints {
...
function findHighestScorer(players: Player[]): JerseyPoints | null {
    if (players.length === 0) {
        return null;
    }

    const jerseyPointsList: JerseyPoints[] = players.map(calculateTotalPoints);

    // Using reduce to find the player with the highest total points
    const highestScorer = jerseyPointsList.reduce((max, current) =>
