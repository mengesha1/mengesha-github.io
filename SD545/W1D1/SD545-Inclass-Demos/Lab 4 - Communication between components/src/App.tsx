...
function ReplayComponent({comm}:{comm:Replay}) {
  
  return(
    <div style={{marginLeft: '10px'}}>
      <div className="reply-item" key={comm.rpid}>
        {/* profile */}
        ...
        <div className="content-wrap">
          ...
          <div className="user-info">
            <div className="user-name">{comm.user.uname}</div>
          </div>
          {/* comment content */}
          <div className="root-reply">
            <span className="reply-content">{comm.content}</span>
            <div className="reply-info">
              {/* comment created time */}
              <span className="reply-time">{comm.ctime}</span>
              {/* total likes */}
              <span className="reply-time">Like:{comm.like}</span>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="reply-list">
            ...
            {commentList.map((item) => (
              <div className="reply-item" key={item.rpid}>
                ...
                {item.replay &&
                  item.replay.map((data) => (
                    <ReplayComponent comm ={data} /> 
                 ))}
              </div>
            ))}
          </div>
